...
function getPurchaseLink(productName) {
  const message = `Привет, меня заинтересовал ${productName}, можно ли его приобрести?`;
...
function renderModalActions(plugin) {
  if (plugin.paid) {
    return `
      <a href="${getPurchaseLink(plugin.name)}" class="btn btn-primary" target="_blank" rel="noopener noreferrer">
        ${createIcon("telegram")} Купить
      </a>
    `;
  }

  return `
    <a href="${plugin.github}" class="btn btn-primary" target="_blank" rel="noopener noreferrer">
      ...
      <div class="flex items-start justify-between gap-4 mb-6">
        <div>
          <div class="flex flex-wrap items-center gap-2 mb-3">
            ${getPriceBadge(plugin)}
            <span class="tag">${CATEGORY_LABELS[plugin.category]}</span>
          </div>
          <h2 id="modal-title" class="font-display text-2xl font-semibold text-neutral-50">${plugin.name}</h2>
